...
public class FFCSVLogger {
	 ...
	 */
	public int init(String subDir, String logPrefix, String[] data_fields, String[] units_fields) {
		...
		try {
			...
			if (Robot.isReal()) {
				log_name = OUTPUT_DIR_RIO + "log_" + DriverStation.getInstance().getEventName() + "_"
						...
						+ "_" + logPrefix + ".csv";
				folderName = OUTPUT_DIR_RIO + "/" + subDir;
			} else {
				log_name = OUTPUT_DIR_LOCAL + "log_" + getDateTimeString() + "_" + logPrefix + ".csv";
				folderName = OUTPUT_DIR_LOCAL + "/" + subDir;
				SmartDashboard.putString("FOLDer", folderName);
			}
